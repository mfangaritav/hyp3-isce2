...
from hyp3_isce2.insar_tops_burst import get_pixel_size, insar_tops_burst_workflow, translate_outputs  # noqa
...
def download_metadata_xmls(params: Iterable[burst_utils.BurstParams], base_dir: Optional[Path] = None) -> None:
    ...
    download_params = [burst_utils.BurstParams(granule, 'IW1', 'VV', 0) for granule in granules]
...
def insar_tops_multi_bursts(references, secondaries, filter_strength: float, apply_water_mask: bool):
    """Run the full ISCE2 workflow for TOPS burst merging
...
def main():
    ...
    products_ref = find_bursts(granules_ref, None, None, None, None, 'IW')
    burst_infos_ref = utils.get_burst_infos(products_ref, work_dir)
    print(f'Found {len(burst_infos_ref)} burst(s).')

    print('Check burst group validity...')
    Safe.check_group_validity(burst_infos_ref)
    
    products_sec = find_bursts(granules_sec, None, None, None, None, 'IW')
    burst_infos_sec = utils.get_burst_infos(products_sec, work_dir)
    print(f'Found {len(burst_infos_sec)} burst(s).')

    print('Check burst group validity...')
    Safe.check_group_validity(burst_infos_sec)
    ...
    if granules_ref[0].split('_')[3] > granules_sec[0].split('_')[3]:
        raise Exception('The secondary granules have a later date than the reference granules')

    product_directory = 'S1_'+granules_ref[0].split('_')[3].split('T')[0]+'_'+granules_sec[0].split('_')[3].split('T')[0]+'_'+granules_sec[0].split('_')[4]
    subprocess.call('mkdir '+product_directory, shell=True)
    cwd = os.getcwd()
    for i,granule_ref in enumerate(granules_ref):
        id_ref = ids_ref[i]
        granule_sec = np.array(granules_sec)[np.array(ids_sec)==id_ref][0]
        granules = [[granule_ref, granule_sec]]
        subprocess.call('mkdir '+product_directory+'/'+id_ref, shell=True)
        os.chdir(product_directory+'/'+id_ref)
        print(granules)
        product_dir, output_zip = insar_tops_burst_workflow(granules, args.looks, args.apply_water_mask)
        subprocess.call('rm -rf *.zip', shell=True)
        subprocess.call('mv S1_* ../', shell=True)
        os.chdir(cwd)
        subprocess.call('rm -rf '+product_directory+'/'+id_ref, shell=True)

    product_dir = Path(product_directory)
    merge_tops_bursts(product_dir, args.filter_strength, args.apply_water_mask)
